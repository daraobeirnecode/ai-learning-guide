...
Three taxonomies cover most of what you need: OWASP LLM Top 10 (an engineering-oriented vulnerability list), MITRE ATLAS (a kill-chain knowledge base for adversarial ML), and NIST AI RMF + the Generative AI Profile (a governance scaffold). The EU AI Act sits alongside as the legal regime an EU-touching consultancy must consider. Use them together — OWASP for "what could go wrong with this build", ATLAS for "how would an attacker actually chain these", NIST for "what artifact do I produce to prove governance", EU AI Act for "what am I legally on the hook for".
...
[MITRE ATLAS](https://atlas.mitre.org/) — the Adversarial Threat Landscape for AI Systems — is to AI security what MITRE ATT&CK is to enterprise security: a kill-chain knowledge base that names tactics and techniques so defenders can communicate precisely. As of version 5.1 (November 2025) ATLAS has 16 tactics, 84 techniques, and 42 real-world case studies, with major 2025 expansions covering generative AI and agentic systems specifically — including RAG Poisoning, LLM Prompt Crafting, Impersonation, and AI Supply Chain Compromise.
...
For a small consultancy this is liberating, not burdensome. You produce six artifacts per client engagement (AI policy, data-flow diagram, threat model, eval suite, runbook, risk register) and you can show NIST RMF compliance to a buyer's auditor. None of them takes more than half a day to author the first time; subsequent clients reuse the templates.
...
Key 2026 dates: **2 August 2026** is when most remaining provisions become applicable, the Commission's enforcement actions can begin (it can request information, model access, and recalls), Article 50 transparency rules become applicable, and serious-incident reporting kicks in. The GPAI rules already applied from 2 August 2025; the [General-Purpose AI Code of Practice](https://digital-strategy.ec.europa.eu/en/policies/contents-code-gpai) was published 10 July 2025 as a voluntary route to demonstrate compliance.

For an SMB consultant, the practical implication is the **provider vs. deployer** distinction. You are almost never a "provider of a general-purpose AI model" (that's Anthropic, OpenAI, Mistral). You are a **deployer** of AI systems built on someone else's model, and sometimes a **provider** of a downstream AI system (the construction RFI assistant you ship is your AI system, with you as the provider, the client as the deployer). High-risk deployments require a Fundamental Rights Impact Assessment before first use. For construction PM agents on routine paperwork, you are almost certainly in the limited-risk tier — disclose AI involvement, label generated content, you're done. For anything touching hiring, credit, education access, or biometric ID, get a lawyer before signature.
...
You are not your client's DPO. You are responsible for the **processor** obligations and helping the client meet the **controller** obligations. Six things to know.

**Article 28** governs the processor relationship — you need a written Data Processing Addendum with every EU client, listing your sub-processors (Anthropic, OpenAI, hosting providers, n8n cloud if used). **Article 35** requires a Data Protection Impact Assessment for high-risk processing; AI systems making automated decisions about EU residents almost always qualify. **Article 22** gives data subjects the right not to be subject to solely-automated decisions with legal or significant effects, and to demand human review and explanation — this is the EU's "right to explanation" and it is the reason your high-stakes agents need a human-in-the-loop. **Article 32** is data security — encryption at rest and in transit, pseudonymisation where feasible, access control, regular testing. **Article 33** is the 72-hour breach notification — when (not if) an incident happens, you have 72 hours to notify the supervisory authority. **Article 30** is the record of processing activities — keep the data-flow diagrams up to date because the regulator can request them.

### 6.4 Zero-data-retention agreements

Both [Anthropic](https://www.anthropic.com/legal/privacy) and OpenAI offer enterprise-tier ZDR. The mechanics: requests are processed in volatile memory, no logs retained beyond a short abuse-monitoring window (typically 30 days, sometimes zero), no use of customer data for training. For SMB clients, ZDR is the differentiator between "your data went to OpenAI and we hope they don't keep it" and "your data went to OpenAI and we have a contract saying they don't keep it." Bake ZDR into the SOW as a requirement for any client handling Confidential or Restricted data.
